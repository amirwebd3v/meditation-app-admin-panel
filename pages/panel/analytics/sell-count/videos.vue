...
const {videosAnalytics, videosMeta, videosTotalSum} = storeToRefs(useAnalyticStore())
...
<template>
  ...
  <AddConfigurationItem Item="All videos"/>


  <v-data-table-server
      class="mt-8 rounded-lg bg-light-brown-1"
      :items-length="+videosMeta.total"
      :page="videosMeta.current_page"
      :items="[...videosAnalytics.values()]"
      @update:options="load"
      :loading="loading"
      :headers="headers"
  ...
  >
    ...
    <template #item.videos_count="{item}">
  ...
  <div class="float-right mt-8 mr-4">
    <v-row>
      ...
      <v-text-field variant="outlined"
                    color="primary"
                    readonly
                    density="compact">{{ videosTotalSum }}
      </v-text-field>
    </v-row>
  </div>
